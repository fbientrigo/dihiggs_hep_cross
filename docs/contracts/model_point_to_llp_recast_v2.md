# Contract — `model_point_to_llp_recast_v2`

Status: implemented, additive. This contract sits alongside
`model_point_to_llp_recast_v1` (`docs/contracts/model_point_to_llp_recast_contract.md`,
`contracts/model_point_to_llp_recast_v1.yaml`, `src/llp_recast/data_contract.py`).
v1 is untouched: its file, its module, and its semantics are unchanged. See
`docs/v1_to_v2_mapping.md` for the column-by-column relationship between the
two.

Code: `src/llp_recast/data_contract_v2.py`. Machine-readable mirror (written
by that module's `emit()`, never hand-edited):
`contracts/model_point_to_llp_recast_v2.yaml`. Tests:
`tests/test_data_contract_v2.py`.
...
g_hH2H2_GeV,
total_width_GeV, ctau_physical_mm, ctau_response_mm,
...
38 columns. This is a straight cross-check pass of the mission's field list
...
### 2. Physical vs. response lifetime (`ctau_physical_mm`, `ctau_response_mm`)
...
- `ctau_response_mm` for `PHYSICAL_DECAYS_NO_HEAVY_CASCADES` (Variant B)
  **must** equal `ctau_physical_mm` exactly (`rel_tol=1e-9`), because
  Variant B never independently tunes a response lifetime
  (`HIGH_MASS_H2_CONTRACT.md` section 4).
- `ctau_response_mm` for `FACTORIZED_G_ONLY` (Variant A) is **not**
  constrained against `ctau_physical_mm` in either direction: it may differ
  (the normal case, a response study fixing its own scan value) and it is
  also not rejected if it happens to coincide. This is a deliberate,
  explicit relaxation of the upstream `high_mass_point_schema.yaml`'s
  stricter `response_ctau_never_identified` invariant (which rejects a
  bit-for-bit Variant A coincidence unless a campaign config separately
  documents it as intentional) — the mission spec explicitly said "do NOT
  require equality for Variant A rows," which this contract reads as "do
  not check this relationship at all for Variant A," not "require
  inequality." **Flagged here as the one place this task's instructions and
  the upstream schema visibly disagree; the orchestrating session may want
  to revisit this if `dihiggs_llp_recast` needs the stricter check.**
...
`contracts/model_point_to_llp_recast_v1.yaml` and
`src/llp_recast/data_contract.py` are untouched by this work. See
`docs/v1_to_v2_mapping.md` for how an existing v1 consumer maps onto v2.
