...
from llp_recast import data_contract_v2 as dc2
from llp_recast.constants import HBAR_C_GEV_MM

REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
CONTRACT_YAML = os.path.join(
    REPO_ROOT, "contracts", "model_point_to_llp_recast_v2.yaml"
)
...
def _base_row(**overrides):
    row = {
        "schema_version": "model_point_to_llp_recast_v2",
        "point_id": "hmp_0001",
...
def _good_row_variant_a(**overrides):
    row = _base_row(model_variant=dc2.VARIANT_A)
...
def _good_row_variant_b(**overrides):
    row = _base_row(model_variant=dc2.VARIANT_B)
...
def test_committed_yaml_matches_code():
    """contracts/model_point_to_llp_recast_v2.yaml must equal the code
    definition. If this fails, run
    `python -m llp_recast.data_contract_v2 --emit`."""
    yaml = pytest.importorskip("yaml")
    assert os.path.exists(CONTRACT_YAML), "run data_contract_v2 --emit and commit"
    with open(CONTRACT_YAML) as fh:
        committed = yaml.safe_load(fh)
    assert committed == dc2.MODEL_POINT_CONTRACT


def test_does_not_touch_v1():
    """v2 must not mutate v1's contract file or code (sanity guard: this test
    only checks the v1 YAML still parses as the v1 contract name)."""
    yaml = pytest.importorskip("yaml")
    v1_path = os.path.join(REPO_ROOT, "contracts", "model_point_to_llp_recast_v1.yaml")
    with open(v1_path) as fh:
        v1 = yaml.safe_load(fh)
    assert v1["name"] == "model_point_to_llp_recast_v1"
...
def test_rejects_variant_b_response_ctau_mismatch(tmp_path):
    ...
    assert not report.ok
    assert any(
        "ctau_response_mm" in name for name, _c, _e in report.invariant_violations
    )
