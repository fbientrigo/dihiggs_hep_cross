"""Machine-readable + enforceable version of the
``model_point_to_llp_recast_v2`` data contract.

This module is a **new, additive** contract that sits alongside
``data_contract.py`` (v1). It does not replace v1, does not change v1's
semantics, and v1 consumers are unaffected. See ``docs/v1_to_v2_mapping.md``
for the column-by-column mapping from v1 to v2.
...
non-canonical scaling estimate. v2 adds all of these fields (see the mission
field list reproduced in ``docs/contracts/model_point_to_llp_recast_v2.md``).

## Design decisions (see docs/contracts/model_point_to_llp_recast_v2.md for
...
2. **Physical vs. response lifetime are separate columns, never aliased.**
   ...
   ``docs/contracts/model_point_to_llp_recast_v2.md``).
...
CLI:
    ...
    python -m llp_recast.data_contract_v2 --input model_points_v2.csv

Programmatic:

    from llp_recast.data_contract_v2 import validate_csv, MODEL_POINT_CONTRACT
    report = validate_csv("model_points_v2.csv")
    if not report.ok:
...
# v2 introduces no legacy-name aliases of its own (it is a fresh, additive
# contract, not a rename of v1's columns); kept as an empty dict so v2's
# structure/API stays parallel to v1's ALIASES mechanism for any future
...
MODEL_POINT_CONTRACT = {
    ...
    "invariants": [
        ...
        # Full 10-channel BR list (v1 only summed 5).
        {
        ...
        # no constraint either way for Variant A.
        {
            ...
            "columns": ["model_variant", "ctau_physical_mm", "ctau_response_mm"],
            "rel_tol": 1e-9,
        },
...
def _response_ctau_problem(row, inv):
    variant = (row.get("model_variant") or "").strip()
    if variant != VARIANT_B:
        return ""
    ...
    if abs(phys - resp) / scale > inv.get("rel_tol", 1e-9):
        return "ctau_response_mm=%r != ctau_physical_mm=%r (must be exactly equal for %s)" % (
            resp,
            phys,
            VARIANT_B,
        )
...
_KIND_HANDLERS = {
    ...
    "response_ctau_equals_physical_for_variant_b": _response_ctau_problem,
    "decay_owner_consistency": _decay_owner_problem,
}


def validate_csv(path, check_invariants=True):
    """Validate a model-point CSV against MODEL_POINT_CONTRACT (v2). Returns
...
def _repo_root():
    # src/llp_recast/data_contract_v2.py -> repo root is three levels up.
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Validate a model-point CSV against the v2 LLP recast input contract."
    )
    parser.add_argument(
        "--emit",
        action="store_true",
    ...
    parser.add_argument(
        "--no-invariants",
        action="store_true",
        help="check only column presence, skip invariants",
    )
